...
SELECT c.deporte, c.dia, COUNT(i.documentosocio) AS "Cantidad de socios inscriptos" FROM inscritos i
INNER JOIN cursos c ON (i.numero = c.numero)
WHERE COUNT(i.documentosocio) NOT IN (
    SELECT c.deporte, COUNT(i.documentosocio) AS "Cantidad de socios inscriptos" FROM inscritos i
    INNER JOIN cursos c ON i.numero = c.numero
)
...
-- 14- Elimine la vista "vista_inscritos" si existe y créela para que muestre la cantidad de inscritos por curso, incluyendo el número del curso, el nombre del deporte y el día.
